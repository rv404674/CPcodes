...
Int solve(  string& a , Int idx , Int limit , Int small , Int presum  )
{
 ...
 if( small == 1 )  
 { 
     if(  i > num && idx != limit-1 ) 
         sum =  sum +  compute( presum ,  i )  + solve(  a , idx + 1 , limit , 2 , presum + i ) ;
                         if(  i < num  ) sum =  sum +  compute( presum ,  i )  + solve(  a , idx + 1 , limit , 0 , presum + i ) ;                          if(  i == num ) sum =  sum +  compute( presum ,  i )  + solve(  a , idx + 1 , limit , 1 , presum + i ) ;                          }      
       
   }

   dp[ idx ][ small ][presum] =  sum ; 
   return dp[ idx ][ small ][presum] ; 
   }
