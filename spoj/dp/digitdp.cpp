...
#define FOR(i, n) for (int i = 0; i < n; i++) #define Int long long #define mp make_pair Int na , nb ; // na number of digits in a and b  Int dp[ 10 ][ 3 ][ 100 ] ;  // dp[i][j][k] stores the sum  // i - > length of the number formed
...
Int solve(  string& a , Int idx , Int limit , Int small , Int presum  )
{
   ...
   for( ; i <=  loop ; i ++ )      {       if( small == 0 ) { sum =  sum +  compute( presum ,  i )  +  solve(  a , idx + 1 , limit , 0 , presum + i ) ; }       if( small == 2 ) { sum =  sum +  compute( presum ,  i ) + solve(  a , idx + 1 , limit , 2 , presum + i ) ; }       if( small == 1 ) { if(  i > num && idx != limit-1 ) sum =  sum +  compute( presum ,  i )  + solve(  a , idx + 1 , limit , 2 , presum + i ) ;
                         if(  i < num  ) sum =  sum +  compute( presum ,  i )  + solve(  a , idx + 1 , limit , 0 , presum + i ) ;                          if(  i == num ) sum =  sum +  compute( presum ,  i )  + solve(  a , idx + 1 , limit , 1 , presum + i ) ;                         }      }    dp[ idx ][ small ][presum] =  sum ; // storing the value in table     return dp[ idx ][ small ][presum] ; } int main ( ) {           na = nb = 0  ;         Int a , b ;         cin >>  a >> b ;
        if(  a == -1 && b == -1 ) break  ;
        calc( a - 1, b );
